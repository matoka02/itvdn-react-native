import { Canvas, Skia, LinearGradient, Path, Text, useFont, vec }
  ...
  return (
    <View style={styles.container}>
      <Canvas style={styles.canvas}>
        <Path path={newPath}>
          <LinearGradient start={vec(100, 100)} end={vec(120, 225)} colors={['#63C5DA', '#0492C2']} />
        </Path>
        {data.map((dataPoint) => (
          <Text color={'white'} key={dataPoint.label} font={font} x={x(dataPoint.label) - 10} y={CanvasHeight - 25} text={dataPoint.label} />
        ))}
      </Canvas>
    </View>
  );
